...
/** Days between generation and the recommended next compliance review. */
const REVIEW_CADENCE_DAYS = 90;

/**
 * Stable, human-readable document identifier for audit trails. Derived
 * deterministically from the package content (djb2 → base36) so the same
...
export function exportToMarkdown(pkg: CompliancePackage): string {
  ...
  // Enterprise document-control block for audit trails and the recurring-review
  // cadence that underpins ongoing compliance operations.
  lines.push("### Document Control\n");
  lines.push("| Field | Value |");
  lines.push("|---|---|");
  lines.push(`| Document ID | ${documentId(pkg, now)} |`);
  lines.push(`| Version | 1.0 |`);
  lines.push(`| Effective date | ${now} |`);
  lines.push(`| Next scheduled review | ${addDays(now, REVIEW_CADENCE_DAYS)} |`);
  lines.push(`| Prepared by | Comply-Quick compliance engine |`);
  lines.push(`| Overall compliance score | ${pkg.complianceScore.overall}/100 |`);
  lines.push("");
