...
export default async function OpenGraphImage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return new ImageResponse(
    ...
    >
      <div
        style={{ display: "flex", alignItems: "center", gap: "16px", color: "#c7d2fe", fontSize: 28, fontWeight: 700 }}
      >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "24px", maxWidth: "1020px" }}>
        <div style={{ color: "#a5b4fc", fontSize: 24, fontWeight: 700, letterSpacing: "0.12em" }}>
          {category.toUpperCase()}
        </div>
        <div
          style={{ fontSize: title.length > 72 ? 56 : 68, fontWeight: 700, lineHeight: 1.08, letterSpacing: "-0.04em" }}
        >
          {title}
        </div>
      </div>
